        ...
        <div id="tab-group-1">
            <div class="tab-controls tabs-small" data-highlight="bg-blue-dark">
                @foreach($festival->days() as $key => $day)
                    @if($key == 0)
                        <a href="#" data-active data-bs-toggle="collapse" data-bs-target="#tab-{{$key}}">{{ Carbon\Carbon::parse($day)->format('F j')}}</a>
                    @else
                    <a href="#" data-bs-toggle="collapse" data-bs-target="#tab-{{$key}}">{{ Carbon\Carbon::parse($day)->format('F j')}}</a>
                        @endif
                @endforeach
                @foreach($festival->days() as $key => $day)
                    @if($key == 0)
                            ...
                            <div class="card card-style">
                                ...
                                @endif>
                                    ...
                                    <div class="card-bottom ms-3">
                                        ...
                                        <p class="color-white font-5 opacity-80 mb-2"><i class="fa fa-map-marker-alt"></i> {{$performance->stage->name}} {{$performance->stage->lat}}, {{$performance->stage->long}} </p>
                                    </div>
